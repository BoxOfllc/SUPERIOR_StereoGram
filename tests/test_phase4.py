...
class TestComfyUINodes(unittest.TestCase):
    ...
    def test_all_10_nodes_registered(self):
        ...
        expected = [
            "DF_DepthPrep", "DF_PatternGen", "DF_PatternLibrary",
            "DF_Stereogram", "DF_AnaglyphOut", "DF_StereoPair",
            "DF_HiddenImage", "DF_SafetyLimiter", "DF_QCOverlay",
            "DF_VideoSequence",
        ]
    ...
    def test_df_depth_prep_with_params(self):
        ...
        depth_tensor = _make_depth_tensor()
        result = node.execute(
            depth_tensor, invert=True, bilateral_space=3.0,
            bilateral_color=0.05, dilation_px=2, falloff_curve="gamma",
            near_plane=0.1, far_plane=0.9,
        )
        depth_map, preview = result
        self.assertEqual(depth_map.ndim, 2)
    ...
    def test_df_pattern_gen_all_types(self):
        from depthforge.comfyui.nodes import DF_PatternGen
        node = DF_PatternGen()
        types = ["random_noise", "perlin", "plasma", "voronoi",
    ...
    def test_df_stereogram_execute(self):
        from depthforge.comfyui.nodes import DF_Stereogram, DF_PatternGen, DF_DepthPrep
    ...
    def test_df_stereogram_with_preset(self):
        from depthforge.comfyui.nodes import DF_Stereogram, DF_PatternGen
        node = DF_Stereogram()
    ...
    def test_df_qc_overlay_all_types(self):
        ...
        node  = DF_QCOverlay()
        depth = _make_depth(32, 48)
        types = ["parallax_heatmap", "depth_bands", "violation_overlay",
                 "safe_zone", "histogram"]
        for t in types:
    ...
    def test_df_video_sequence_execute(self):
        from depthforge.comfyui.nodes import DF_VideoSequence, DF_PatternGen
...
class TestNukeGizmo(unittest.TestCase):
    ...
    def test_gizmo_render_texture(self):
        from depthforge.nuke.gizmo import DepthForgeGizmo
        g = DepthForgeGizmo({"df_mode": "Texture", "df_pattern_type": "plasma",
                              "df_color_mode": "psychedelic"})
        depth = _make_depth(32, 48)
        result = g.render(depth)
        self.assertEqual(result.shape, (32, 48, 4))
...
class TestWebApp(unittest.TestCase):
    ...
    def _post_synthesize(self, mode="texture", extra=None):
        ...
        if extra:
            body.update(extra)
        return self.client.post("/api/synthesize",
    ...
    def test_synthesize_hidden(self):
        r = self._post_synthesize("hidden", {
            "hidden_shape": "star",
            "tile_size": 48,
        })
        self.assertEqual(r.status_code, 200)
    ...
    def test_synthesize_anaglyph(self):
        source = _make_rgba(32, 48)
        r = self._post_synthesize("anaglyph", {
    ...
    def test_synthesize_missing_depth_400(self):
        r = self.client.post("/api/synthesize",
            data=json.dumps({"mode": "texture"}),
            content_type="application/json")
    ...
    def test_analyze_returns_report(self):
        ...
        self.assertEqual(r.status_code, 200)
        data = r.get_json()
        for key in ["passed", "overall_score", "max_parallax_px", "max_vergence_deg",
                    "violations", "advice", "heatmap_b64", "overlay_b64"]:
    ...
    def test_analyze_missing_depth_400(self):
        r = self.client.post("/api/analyze",
            data=json.dumps({"depth_factor": 0.3}),
            content_type="application/json")
        self.assertEqual(r.status_code, 400)
    ...
    def test_estimate_depth_mock(self):
        ...
        r    = self.client.post("/api/estimate_depth",
            data=json.dumps({
    ...
    def test_qc_overlay_heatmap(self):
        depth = _make_depth(32, 48)
        r = self.client.post("/api/qc_overlay",
            data=json.dumps({
                "depth_b64":    _depth_to_b64(depth),
...
class TestPhase4Integration(unittest.TestCase):
    ...
    def test_comfyui_node_pipeline(self):
        ...
        depth_map, _ = DF_DepthPrep().execute(
            raw_depth, bilateral_space=3.0, falloff_curve="gamma"
        )
        pattern, _, _ = DF_PatternLibrary().execute("northern_lights", 48, 48, 42, 1.0)
        result, = DF_Stereogram().execute(
            depth_map, pattern, depth_factor=0.3, safe_mode=True
        )
        self.assertIsNotNone(result)
