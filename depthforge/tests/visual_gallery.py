...
from depthforge.core.pattern_gen   import (
    PatternParams, PatternType, ColorMode, GridStyle, generate_pattern, tile_to_frame,
)
...
from depthforge.core.hidden_image import HiddenImageParams, encode_hidden_image, shape_to_mask, text_to_mask
from depthforge.core.adaptive_dots import AdaptiveDotParams, generate_adaptive_dots
from depthforge.core.inpainting   import InpaintParams, InpaintMethod, inpaint_occlusion
...
def make_mosaic(images: list, cols: int, padding: int = 4) -> np.ndarray:
    ...
    canvas = np.full(((H + padding) * rows + padding,
                      (W + padding) * cols + padding, 3), 15, dtype=np.uint8)
...
pattern_configs = [
    ("SIRDS Random (Grey)",       PatternParams(PatternType.RANDOM_NOISE, 128, 128, ColorMode.GREYSCALE, seed=0)),
    ("SIRDS Random (Mono Blue)",  PatternParams(PatternType.RANDOM_NOISE, 128, 128, ColorMode.MONOCHROME, hue=0.6, seed=0)),
    ("SIRDS Random (Psychedelic)",PatternParams(PatternType.RANDOM_NOISE, 128, 128, ColorMode.PSYCHEDELIC, seed=0)),
    ("Perlin Noise (Grey)",       PatternParams(PatternType.PERLIN, 128, 128, ColorMode.GREYSCALE, seed=1)),
    ("Perlin (Psychedelic)",      PatternParams(PatternType.PERLIN, 128, 128, ColorMode.PSYCHEDELIC, seed=1)),
    ...
    ("Voronoi (Psychedelic)",     PatternParams(PatternType.VORONOI, 128, 128, ColorMode.PSYCHEDELIC, seed=3)),
    ("Dots Grid",                 PatternParams(PatternType.GEOMETRIC_GRID, 128, 128, grid_style=GridStyle.DOTS, grid_spacing=12)),
    ("Hex Grid",                  PatternParams(PatternType.GEOMETRIC_GRID, 128, 128, grid_style=GridStyle.HEXES, grid_spacing=16)),
    ("Checkerboard",              PatternParams(PatternType.GEOMETRIC_GRID, 128, 128, grid_style=GridStyle.CHECKS, grid_spacing=16)),
    ("Stripes",                   PatternParams(PatternType.GEOMETRIC_GRID, 128, 128, grid_style=GridStyle.STRIPES, grid_spacing=12)),
    ("Mandelbrot",                PatternParams(PatternType.MANDELBROT, 128, 128, ColorMode.GREYSCALE)),
    ("Mandelbrot (Psychedelic)",  PatternParams(PatternType.MANDELBROT, 128, 128, ColorMode.PSYCHEDELIC)),
...
sirds_outputs = []
for depth_name, depth in [("Sphere", DEPTH_SPHERE_P), ("Multi-Sphere", DEPTH_MULTI_P), ("Gradient", DEPTH_GRADIENT_P)]:
    for color_name, color_mode in [("Monochrome", ColorMode.GREYSCALE), ("Psychedelic", ColorMode.PSYCHEDELIC)]:
        pattern = generate_pattern(PatternParams(PatternType.RANDOM_NOISE, 64, 64,
                                                  color_mode, seed=42))
...
for mode in AnaglyphMode:
    out = make_anaglyph_from_depth(src, DEPTH_SPHERE_P,
                                   AnaglyphParams(mode=mode, parallax_px=12))
...
for depth_name, depth in [("Sphere", DEPTH_SPHERE_P), ("Multi-Sphere", DEPTH_MULTI_P)]:
    L, R, occ = make_stereo_pair(src, depth,
                                  StereoPairParams(max_parallax_fraction=0.06,
                                                   feather_px=4))
...
hidden_outputs = []
pattern_grey  = generate_pattern(PatternParams(PatternType.RANDOM_NOISE, 64, 64,
                                                ColorMode.GREYSCALE, seed=7))
pattern_psych = generate_pattern(PatternParams(PatternType.PLASMA, 128, 128,
                                                ColorMode.PSYCHEDELIC, seed=7))
...
for shape_name, mask in [
    ...
    # Monochrome
    out_grey = encode_hidden_image(pattern_grey, mask,
                                   HiddenImageParams(foreground_depth=0.85,
    ...
    # Psychedelic pattern
    out_psych = encode_hidden_image(pattern_psych, mask,
                                    HiddenImageParams(foreground_depth=0.85,
                                                      background_depth=0.0,
                                                      edge_soften_px=6))
...
for depth_name, depth in [("Sphere", DEPTH_SPHERE_P), ("Multi-Sphere", DEPTH_MULTI_P), ("Text", DEPTH_TEXT)]:
    params = AdaptiveDotParams(
        tile_width=SIZE, tile_height=SIZE,
        min_dot_radius=1, max_dot_radius=4,
        min_spacing=3, max_spacing=14, seed=0
    )
...
for curve_name, curve in [
    ...
    p = DepthPrepParams(bilateral_sigma_space=3, dilation_px=2,
                        falloff_curve=curve, falloff_gamma=gamma)
...
pattern_grey = generate_pattern(PatternParams(PatternType.RANDOM_NOISE, 64, 64, ColorMode.GREYSCALE, seed=0))
...
pattern_psych = generate_pattern(PatternParams(PatternType.PLASMA, 128, 128, ColorMode.PSYCHEDELIC, seed=0))
...
# Adaptive dots
adot = generate_adaptive_dots(DEPTH_MULTI_P, AdaptiveDotParams(
    tile_width=SIZE, tile_height=SIZE, seed=0))
