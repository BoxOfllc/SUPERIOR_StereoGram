...
class TestSynthesizer(unittest.TestCase):
    ...
    def test_invert_depth_changes_result(self):
        ...
        r2 = synthesize(
            depth,
            pattern,
            StereoParams(seed=1, depth_factor=0.8, convergence=0.0, invert_depth=True),
        )
    ...
    def test_safe_mode_limits_parallax(self):
        ...
        self.assertLessEqual(p2.max_parallax_fraction, 1.0 / 30.0 + 1e-9)
...
class TestPatternGen(unittest.TestCase):
    ...
    def test_geometric_dots(self):
        p = PatternParams(
            PatternType.GEOMETRIC_GRID, 64, 64, grid_style=GridStyle.DOTS, grid_spacing=8
        )
        t = generate_pattern(p)
        self._check_rgba_tile(t, 64, 64)
    ...
    def test_geometric_checks(self):
        p = PatternParams(
            PatternType.GEOMETRIC_GRID, 64, 64, grid_style=GridStyle.CHECKS, grid_spacing=8
        )
        t = generate_pattern(p)
        self._check_rgba_tile(t, 64, 64)
...
class TestAnaglyph(unittest.TestCase):
    ...
    def test_grey_anaglyph_is_greyscale_channels(self):
        ...
        L = np.zeros((8, 8, 4), dtype=np.uint8)
        L[:, :, 0] = 200
        L[:, :, 3] = 255
        R = np.zeros((8, 8, 4), dtype=np.uint8)
        R[:, :, 1] = 150
        R[:, :, 3] = 255
...
class TestAdaptiveDots(unittest.TestCase):
    ...
    def test_dots_contain_non_bg_pixels(self):
        ...
        params = AdaptiveDotParams(
            tile_width=64, tile_height=64, dot_color=(255, 255, 255), bg_color=(0, 0, 0), seed=7
        )
        tile = generate_adaptive_dots(depth, params)
...
class TestInpainting(unittest.TestCase):
    ...
    def test_clean_plate_method(self):
        ...
        plate[:, :, 2] = 255
        plate[:, :, 3] = 255  # blue plate
        mask = np.zeros((32, 32), dtype=np.float32)
        mask[10:20, 10:20] = 1.0
        p = InpaintParams(
            method=InpaintMethod.CLEAN_PLATE, clean_plate=plate, dilate_mask_px=0, blend_px=0
        )
    ...
    def test_edge_extend_method(self):
        ...
        p = InpaintParams(method=InpaintMethod.EDGE_EXTEND, dilate_mask_px=0, blend_px=0)
        out = inpaint_occlusion(img, mask, p)
    ...
    def test_ai_callback_called(self):
        ...
        p = InpaintParams(
            method=InpaintMethod.AI_CALLBACK, ai_callback=mock_ai, dilate_mask_px=0, blend_px=0
        )
        inpaint_occlusion(img, mask, p)
        self.assertEqual(called["n"], 1)

    def test_auto_selects_clean_plate_when_available(self):
        from depthforge.core.inpainting import InpaintMethod, _resolve_method
...
class TestIntegration(unittest.TestCase):
    ...
    def test_full_pipeline_sirds(self):
        ...
        pattern = generate_pattern(
            PatternParams(PatternType.RANDOM_NOISE, 32, 32, ColorMode.GREYSCALE, seed=0)
        )
    ...
    def test_full_pipeline_anaglyph(self):
        ...
        out = make_anaglyph_from_depth(src, depth, AnaglyphParams(mode=AnaglyphMode.OPTIMISED))
        self.assertEqual(out.shape, (48, 64, 4))
    ...
    def test_full_pipeline_stereo_with_inpaint(self):
        ...
        from depthforge.core.inpainting import InpaintMethod, InpaintParams, inpaint_occlusion
        from depthforge.core.stereo_pair import make_stereo_pair

        L, R, occ = make_stereo_pair(src, depth)
        L_fill = inpaint_occlusion(
            L, occ, InpaintParams(method=InpaintMethod.EDGE_EXTEND, blend_px=0, dilate_mask_px=0)
        )
    ...
    def test_full_pipeline_adaptive_dots(self):
        ...
        tile = generate_adaptive_dots(
            depth, AdaptiveDotParams(tile_width=32, tile_height=32, seed=0)
        )
