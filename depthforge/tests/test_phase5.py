...
def _pattern():
    ...
    return generate_pattern(PatternParams(
        ...
        tile_width=64, tile_height=64, seed=42,
    ))
...
class TestParallelSynthesize(unittest.TestCase):
    ...
    def test_progress_callback_total_correct(self):
        from depthforge.core.parallel import parallel_synthesize, ParallelConfig
        totals = []
        def cb(done, total): totals.append(total)
        cfg = ParallelConfig(n_workers=2, chunk_rows=32, min_rows_for_parallel=1,
                             show_progress=True, progress_callback=cb)
        parallel_synthesize(self.depth, self.pattern, self.params, cfg)
...
class TestComputeFlow(unittest.TestCase):
    ...
    def test_rgba_frames_accepted(self):
        from depthforge.core.optical_flow import compute_flow
        rgba = np.concatenate([self.a, np.full((*self.a.shape[:2], 1), 255, dtype=np.uint8)], axis=-1)
...
class TestFlowToDepth(unittest.TestCase):
    ...
    def test_output_shape(self):
        from depthforge.core.optical_flow import flow_to_depth, FlowDepthConfig
...
class TestTemporalSmootherEMA(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.smoother = TemporalSmoother(TemporalConfig(strategy="ema", alpha=0.5,
...
class TestTemporalSmootherWindowed(unittest.TestCase):
    ...
    def setUp(self):
        from depthforge.core.temporal import TemporalSmoother, TemporalConfig
        self.smoother = TemporalSmoother(TemporalConfig(strategy="windowed", window_size=3,
                                                        scene_cut_reset=False))
...
class TestTemporalSmootherFlowGuided(unittest.TestCase):
    ...
    def setUp(self):
        from depthforge.core.temporal import TemporalSmoother, TemporalConfig
        self.smoother = TemporalSmoother(TemporalConfig(strategy="flow_guided",
                                                        scene_cut_reset=False))
...
class TestTemporalSmootherAdaptive(unittest.TestCase):
    ...
    def test_adaptive_runs(self):
        from depthforge.core.temporal import TemporalSmoother, TemporalConfig
        smoother = TemporalSmoother(TemporalConfig(strategy="adaptive",
                                                   scene_cut_reset=False))
        for i in range(5):
...
class TestSceneCutReset(unittest.TestCase):
    ...
    def test_scene_cut_resets_state(self):
        from depthforge.core.temporal import TemporalSmoother, TemporalConfig
        smoother = TemporalSmoother(TemporalConfig(strategy="ema", alpha=0.1,
...
class TestBuiltinProfiles(unittest.TestCase):
    ...
    def test_all_builtin_accessible(self):
        from depthforge.core.export import list_profiles, get_profile
    ...
    def test_register_custom(self):
        from depthforge.core.export import ExportProfile, register_profile, get_profile
...
class TestPhase5Pipeline(unittest.TestCase):
    ...
    def test_flow_temporal_export(self):
        ...
        smoother = TemporalSmoother(TemporalConfig(strategy="ema", alpha=0.4,
                                                   scene_cut_reset=False))
