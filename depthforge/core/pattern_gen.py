...
class PatternParams:
    ...
    def __post_init__(self) -> None:
        if self.tile_width < 4:
            raise ValueError("tile_width must be >= 4")
        if self.tile_height < 4:
            raise ValueError("tile_height must be >= 4")
...
def _hsv_to_rgb_single(
    ...
    lut = [(v_arr, t, p), (q, v_arr, p), (p, v_arr, t), (p, q, v_arr), (t, p, v_arr), (v_arr, p, q)]
    r, g, b = lut[i]
