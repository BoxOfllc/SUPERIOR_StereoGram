...
from depthforge.core.pattern_cache import PatternCache, cached_pattern
from depthforge.core.pattern_gen   import PatternParams, PatternType, ColorMode
from depthforge.core.pattern_library import get_pattern
from depthforge.core.presets       import get_preset, list_presets
from depthforge.core.lut           import get_builtin_lut, list_builtin_luts, apply_lut
...
def describe_in_context(context: str) -> dict:
    ...
    params = [
        # ── Synthesis controls ───────────────────────────────────────────
        {
            "name": "preset_name", "type": "choice",
            "label": "Preset", "hint": "Named synthesis preset",
            "choices": ["none"] + list_presets(), "default": "none",
            "group": "Synthesis",
        },
        {
            "name": "depth_factor", "type": "double",
            "label": "Depth Factor", "hint": "Parallax strength (0–1)",
            "min": 0.0, "max": 1.0, "default": 0.35,
            "group": "Synthesis",
        },
        {
            "name": "max_parallax", "type": "double",
            "label": "Max Parallax", "hint": "Max parallax as fraction of frame width",
            "min": 0.005, "max": 0.1, "default": 0.033,
            "group": "Synthesis",
        },
        {
            "name": "oversample", "type": "int",
            "label": "Oversample", "hint": "Supersampling factor (1–4)",
            "min": 1, "max": 4, "default": 1,
            "group": "Synthesis",
        },
        {
            "name": "safe_mode", "type": "bool",
            "label": "Safe Mode", "hint": "Hard-clamp parallax to max_parallax",
            "default": False,
            "group": "Synthesis",
        },
        {
            "name": "invert_depth", "type": "bool",
            "label": "Invert Depth", "hint": "Flip near/far convention",
            "default": False,
            "group": "Synthesis",
        },
        {
            "name": "seed", "type": "int",
            "label": "Random Seed", "hint": "Pattern tiling seed",
        ...
        # ── Pattern controls ─────────────────────────────────────────────
        {
            "name": "pattern_source", "type": "choice",
            "label": "Pattern Source", "hint": "Where to get the pattern tile",
            "choices": ["clip", "library", "procedural"], "default": "procedural",
            "group": "Pattern",
        },
        {
            "name": "library_pattern", "type": "choice",
            "label": "Library Pattern",
            "choices": ["plasma_wave", "fine_grain", "linen", "marble",
                        "perlin_noise", "water_ripples", "hexgrid", "neon_grid"],
            "default": "plasma_wave",
            "group": "Pattern",
        },
        {
            "name": "pattern_type", "type": "choice",
        ...
        },
        {
            ...
            "label": "Tile Size", "hint": "Pattern tile size in pixels (square)",
        ...
        # ── Depth prep ───────────────────────────────────────────────────
        {
            "name": "bilateral_space", "type": "double",
            "label": "Smooth (Spatial)", "hint": "Bilateral filter spatial sigma",
            "min": 0.0, "max": 30.0, "default": 5.0,
            "group": "Depth Prep",
        },
        {
            "name": "dilation_px", "type": "int",
            "label": "Dilation (px)", "hint": "Morphological dilation radius",
            "min": 0, "max": 20, "default": 3,
            "group": "Depth Prep",
        },
        {
            "name": "near_plane", "type": "double",
            "label": "Near Plane", "hint": "Clip depth below this value",
            "min": 0.0, "max": 1.0, "default": 0.0,
            "group": "Depth Prep",
        },
        {
            "name": "far_plane", "type": "double",
            "label": "Far Plane", "hint": "Clip depth above this value",
        ...
        {
            "name": "lut_strength", "type": "double",
            "label": "LUT Strength", "hint": "Blend between original and LUT (0–1)",
            "min": 0.0, "max": 1.0, "default": 1.0,
            "group": "Colour Grade",
        },
        ...
        # ── Export profile ───────────────────────────────────────────────
        {
            "name": "export_profile", "type": "choice",
...
def get_pattern_names() -> list:
    from depthforge.core.pattern_library import list_patterns, list_categories
    names = []
...
def render(params_json: str) -> bytes:
    ...
    if pattern_source == "clip" and params.get("pattern_bytes"):
        ...
        pattern  = _cache.get_library(lib_name, tile_sz, tile_sz,
                                      seed=int(params.get("seed", 42)))
    else:
        tile_sz = int(params.get("tile_size", 128))
        pp = PatternParams(
            pattern_type=PatternType(params.get("pattern_type", "random_noise")),
            tile_width=tile_sz, tile_height=tile_sz,
            seed=int(params.get("seed", 42)),
        )
...
if __name__ == "__main__":
    ...
    print(f"  render result: {result.shape} dtype={result.dtype} "
