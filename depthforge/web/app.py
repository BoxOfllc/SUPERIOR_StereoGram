...
def create_app(config: Optional[dict] = None) -> Flask:
    ...
    def api_patterns():
        from depthforge.core.pattern_library import list_patterns, list_categories, get_pattern_info
    ...
    def api_pattern_preview(name):
        ...
        try:
            ...
            return jsonify({"name": name, "image": _array_to_b64(pat), "width": size, "height": size})
        except KeyError:
    ...
    def api_analyze():
        ...
        try:
            ...
            from depthforge.core.qc import parallax_heatmap, QCParams
            ...
            strain   = estimate_vergence_strain(depth, depth_factor=depth_factor, frame_width=frame_width)
    ...
    def api_qc_overlay():
        ...
        try:
            from depthforge.core.qc import (
                parallax_heatmap, depth_band_preview, depth_histogram,
                window_violation_overlay, safe_zone_indicator, QCParams
            )
            depth        = _b64_to_depth(body["depth_b64"])
...
def _synthesize(body: dict, progress_cb=None) -> dict:
    ...
    if H > _MAX_DIM or W > _MAX_DIM:
        raise ValueError(
            f"Input image too large ({W}×{H}). Maximum dimension is {_MAX_DIM}px."
        )
    ...
    if pattern_name:
        ...
        pattern   = get_pattern(pattern_name, width=tile_size, height=tile_size,
        ...
        pattern = generate_pattern(PatternParams(
            ...
            color_mode=color_map.get(body.get("color_mode", "psychedelic"), ColorMode.PSYCHEDELIC),
            seed=int(body.get("seed", 42)),
        ))
    ...
    if mode in ("sirds", "texture"):
        ...
        from depthforge.core.stereo_pair import (
            make_stereo_pair, compose_side_by_side, StereoPairParams
        )
        source_b64 = body.get("source_b64")
        if not source_b64:
